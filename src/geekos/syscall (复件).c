...
static int old_Copy_User_String(ulong_t uaddr, ulong_t len,
...
{     
    ...
    /* 从用户内存空间中复制字符串到系统内核空间 */     
    if (Copy_From_User(str, uaddr, len) == 0)     
    {        
	    Free(str);         
	    return EINVALID;     
	}     
 ...
 */
static int old_Sys_PrintString(struct Interrupt_State* state)
{
 ...
 */
static int old_Sys_Spawn(struct Interrupt_State* state)
{
...
{     
    ...
    /* 生成用户进程 */     
    Enable_Interrupts();     
    res = Spawn(program, command, &process);     
    if (program != NULL) Free(program);     
    if (command != NULL) Free(command);     
    Disable_Interrupts();     
    if (res != 0) return res;     
 ...
 */
static int old_Sys_Wait(struct Interrupt_State* state)
{
